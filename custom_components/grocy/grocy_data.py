        ...
        def wrapper() -> list[Chore]:
            try:
                ...
                if "Expecting value" in error_msg or "JSON" in error_msg:
                    _LOGGER.debug(
                        "Empty or invalid JSON response from Grocy chores API, returning empty list: %s",
                        error_msg,
                    )
                    return []
                raise
    ...
    async def async_update_overdue_chores(self):
        ...
        def wrapper():
            try:
                ...
                if "Expecting value" in error_msg or "JSON" in error_msg:
                    _LOGGER.debug(
                        "Empty or invalid JSON response from Grocy overdue chores API, returning empty list: %s",
                        error_msg,
                    )
                    return []
                raise
